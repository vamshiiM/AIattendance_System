...
class Attendence:
    def __init__(self, root):
        ...
        title_lev_ = Label(self.root, text="Attendence Details", font=('times new roman', 45, 'bold'), bg="skyblue",
                           fg="white")
        ...
        ## creating left frame
        left_frame = LabelFrame(main_frame, bd=2, bg='white', relief=RIDGE, text="Student Attendence Details",
                                font=('times new roman', 12, 'bold'))
        left_frame.place(x=20, y=10, width=650, height=580)

        img_left = Image.open(r'E:\student_management\photo\leon-wu-LLfRMRT-9AY-unsplash.jpg')
        img_left = img_left.resize((700, 190), Image.ANTIALIAS)
        self.photoimg_left = ImageTk.PhotoImage(img_left)

        left_lev = Label(left_frame, image=self.photoimg_left)
        left_lev.place(x=1, y=0, width=700, height=190)
        ...
        ## leble and entry
        # attendence_id
        attendence_id_lable = Label(left_indiseframe, text="Attendence ID : ", font=('times new roman', 13, 'bold'),
                                    bg='white')
        ...
        # Student Roll
        student_roll_lable = Label(left_indiseframe, text="Student Roll : ", font=('times new roman', 13, 'bold'),
                                   bg='white')
        ...
        # Student name
        student_name_lable = Label(left_indiseframe, text="Student Name : ", font=('times new roman', 13, 'bold'),
                                   bg='white')
        ...
        dep_combo["values"] = (
        "Select Department", "Computer Science", "ECE", "IT", "Civil", "Mechanical", "Data Science")
        dep_combo.current(0)
        dep_combo.grid(row=10, column=3, padx=2, pady=7, sticky=W)

        # time
        attendence_time_lable = Label(left_indiseframe, text="Time : ", font=('times new roman', 13, 'bold'),
                                      bg='white')
        ...
        # Date
        attendence_date_lable = Label(left_indiseframe, text="Date : ", font=('times new roman', 13, 'bold'),
                                      bg='white')
        ...
        # Attendence label
        attendence_lable = Label(left_indiseframe, text="Attendence Status :", font=('times new roman', 13, 'bold'),
                                 bg='white')
        ...
        # Reset
        reset_botton = Button(btn_frame, text="Update", width=30, bg='white', fg='Black',
                              font=('times new roman', 13, 'bold'))
        ...
        # btn_frame1 = Frame(left_indiseframe,bd=2,relief=RIDGE,bg="white")
    ...
    def importCSV(self):
        global mydata
        mydata.clear()
        fln = filedialog.askopenfilename(initialdir=os.getcwdb(), title="Open CSV",
    ...
    def exportCSV(self):
        try:
            if len(mydata) < 1:
                ...
                return False
            fln = filedialog.asksaveasfilename(initialdir=os.getcwdb(), title="Open CSV",
                                               filetypes=(("CSV File", "*.csv"), ("All File", "*.*")), parent=self.root)
